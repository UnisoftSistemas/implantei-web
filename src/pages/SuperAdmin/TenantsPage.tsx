...
export const TenantsPage = () => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async ({ id, active }: { id: string; active: boolean }) => {
      await apiClient.put(`/companies/${id}`, { active });
    },
    onSuccess: () => {
  ...
  return (
    <VStack gap={8} align="stretch">
      ...
      <HStack gap={6}>
        <Box>
          <Text fontSize="2xl" fontWeight="bold" color="gray.800">
            {tenantsData?.pagination.total || 0}
          </Text>
        ...
        <Box>
          <Text fontSize="2xl" fontWeight="bold" color="green.600">
            {tenantsData?.data.filter((t) => t.active).length || 0}
          </Text>
          <Text fontSize="sm" color="gray.500">
                ...
                <VStack align="stretch" gap={4}>
                  ...
                  <HStack gap={2} pt={2}>
                    <Button
                      ...
                      flex={1}
                    >
                      <Users size={14} /> {t("superAdmin.tenants.manageUsers")}
                    </Button>
                    ...
                    >
                      <Activity size={14} />{" "}
                      {t("superAdmin.tenants.viewProjects")}
                    </Button>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleToggleStatus(tenant.id, tenant.active)
                      }
      ...
      {/* Pagination */}
      {tenantsData && tenantsData.pagination.total > 12 && (
        <HStack justify="center" gap={2}>
          ...
          <Text fontSize="sm" color="gray.600" px={4}>
            {t("common.pageOf", {
              current: page,
              total: Math.ceil(tenantsData.pagination.total / 12),
            })}
          </Text>
          ...
          <Button
            size="sm"
            variant="outline"
            disabled={page >= Math.ceil(tenantsData.pagination.total / 12)}
            onClick={() => setPage(page + 1)}
          >
            {t("common.next")}
          </Button>
